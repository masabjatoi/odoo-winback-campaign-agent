...
7. **Evaluate Drip Campaign Timing & Action:**
   ...
   - If current time IS past `next_email_date`:
       ...
       - **Language Enforcement:** When calling `email_copywriter`, always include the exact `lang` value and `country` value from the lead state in your task description. Explicitly instruct the copywriter which target language to use (e.g. "The customer's lang is nl_BE and country is Belgium — write the entire email in Dutch"). After the copywriter returns the draft, verify the email body language matches the expected target language before proceeding. If it does not match, re-invoke the copywriter with a corrected explicit language instruction.
     - **If stage is 'none':**
